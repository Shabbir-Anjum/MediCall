...
import { Phone, Clock, User, AlertTriangle, CheckCircle, XCircle, Filter, Download, Eye } from 'lucide-react';
...
export default function CallHistoryPage() {
  ...
  const getOutcomeColor = (outcome: string) => {
    switch (outcome) {
      ...
      case 'transferred-emergency': return 'bg-red-100 text-red-800';
      case 'appointment-requested': return 'bg-blue-100 text-blue-800';
  ...
  const getCallTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'follow-up': return 'bg-purple-50 text-purple-700';
  ...
  const getOutcomeIcon = (outcome: string) => {
    switch (outcome) {
      ...
      case 'no-answer': return <XCircle className="h-4 w-4 text-yellow-600" />;
      case 'busy': return <Clock className="h-4 w-4 text-orange-600" />;
      case 'transferred-emergency': return <AlertTriangle className="h-4 w-4 text-red-600" />;
      case 'appointment-requested': return <User className="h-4 w-4 text-blue-600" />;
                            ...
                            <div className="flex items-center space-x-3">
                              <Avatar className="h-8 w-8">
                                ...
                                <AvatarFallback className="text-xs">
                                  {log.patient.name.split(' ').map(n => n[0]).join('')}
                                </AvatarFallback>
                              </Avatar>
                              <div>
                          ...
                          <TableCell>
                            <div className="flex items-center space-x-2">
                              <Avatar className="h-6 w-6">
                                ...
                                <AvatarFallback className="text-xs">
                                  {log.agent.name.split(' ').map(n => n[0]).join('')}
                                </AvatarFallback>
                              </Avatar>
